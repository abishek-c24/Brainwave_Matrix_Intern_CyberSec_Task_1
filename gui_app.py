...
class PhishingScannerApp(ctk.CTk):
    def __init__(self):
        ...
        # Loading Spinner
        self.loading_spinner = CTkProgressBar(self, mode="indeterminate", width=300)
    ...
    def scan_url(self):
        ...
        if url:
            threading.Thread(target=self.scan_single_url, args=(url,), daemon=True).start()
        else:
    ...
    def scan_from_file(self):
        ...
        try:
            ...
            if not urls:
                ...
                return
            for url in urls:
                threading.Thread(target=self.scan_multiple_urls, args=(urls,), daemon=True).start()
    ...
    def display_result(self, result):
        ...
        if "Safe" in status:
            tag = "green"
        elif "Malicious" in status or "Invalid" in status:
            tag = "red"
        else:
            tag = "orange"
